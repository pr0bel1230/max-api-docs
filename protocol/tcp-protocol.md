...
opcode      = 64-92       # 2 байта — код операции
...
- `flags & 0x1` — наличие ведущих ints (всегда 1 для ответов сервера)
...
def recv_frame(sock):
    ...
    _, cmd, _, opcode, packed = struct.unpack(">BBHHI", header)
    plen = packed & 0x00FFFFFF
    payload = recv_exact(sock, plen) if plen else b""
    return {'cmd': cmd, 'opcode': opcode, 'payload': payload}
```
